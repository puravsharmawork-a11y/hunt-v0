      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '28px 40px 60px' }}>
        ...
        {activeSection === 'overview' && (
          <div style={{ maxWidth: 760 }}>
            {/* Banner + Avatar hero */}
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 0, overflow: 'hidden', marginBottom: 20 }}>
              {/* Banner */}
              <div style={{
                height: 160,
                ...
                {!d.banner_url && (
                  <p style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 28, color: 'var(--cream)', fontStyle: 'italic', letterSpacing: '0.02em' }}>On a hunt.</p>
                )}
              ...
              <div style={{ padding: '0 28px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ position: 'relative', marginTop: -54 }}>
                    {d.avatar_url ? (
                      <img src={d.avatar_url} alt="avatar" style={{ width: 108, height: 108, border: '4px solid var(--bg-card)', objectFit: 'cover', display: 'block' }} />
                    ) : (
                      <div style={{ width: 108, height: 108, background: 'var(--ink)', border: '4px solid var(--bg-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'JetBrains Mono', monospace", fontSize: 36, fontWeight: 600, color: 'var(--cream)', letterSpacing: '0.04em' }}>
                        {initials}
                      </div>
                    )}
                    <label style={{ position: 'absolute', bottom: 4, right: 4, width: 28, height: 28, background: 'var(--blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid var(--bg-card)' }}>
                ...
                <p style={{ fontFamily: "'Instrument Serif', Georgia, serif", fontSize: 22, color: 'var(--text)', marginBottom: 3 }}>{d.full_name || '—'}</p>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        {[
                          ...
                          <button key={opt.v} onClick={() => setDraft(x => ({ ...x, user_type: opt.v }))} style={{
                            padding: '7px 14px', borderRadius: 0, fontSize: 12, fontFamily: 'inherit', cursor: 'pointer',
                ...
                return (
                  <>
                    ...
                    {contactFields.map(({ key, label, ph, icon, color }) => (
                      <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8 }}>
                        <div style={{ width: 28, height: 28, borderRadius: 0, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 13 }}>{icon}</div>
        ...
        {activeSection === 'resume' && (
          <div style={{ maxWidth: 700 }}>
              ...
              {({ editing }) => (
                <>
                  {[
                    ...
                    <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 0, background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'white' }}>{logo}</div>
                      <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-mid)', width: 90, flexShrink: 0 }}>{label}</span>
                      {editing ? (
            ...
            <Card>
              ...
              {d.resume_url ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderRadius: 0, border: '1px solid var(--green)', background: 'var(--green-tint)' }}>
                  ...
                  <label style={{ fontSize: 12, padding: '5px 12px', borderRadius: 0, border: '1px solid var(--green)', color: 'var(--green)', cursor: 'pointer', fontFamily: 'inherit' }}>
                    Replace<input type="file" accept=".pdf" style={{ display: 'none' }} onChange={handleResumeUpload} />
                  </label>
                </div>
              ) : (
                <label style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '32px', borderRadius: 0, border: '2px dashed var(--border-mid)', cursor: 'pointer' }}
            ...
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                ...
                <button onClick={() => setAddingEdu(true)} style={{ padding: '6px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-dim)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>+ Add</button>
              </div>
              {(d.education || []).map((edu, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, padding: '12px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8, alignItems: 'flex-start' }}>
                  <div style={{ width: 36, height: 36, borderRadius: 0, background: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16 }}>🎓</div>
                  <div style={{ flex: 1 }}>
              ...
              {addingEdu && (
                <div style={{ border: '1px solid var(--border)', borderRadius: 0, padding: 16, background: 'var(--bg-subtle)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => { if (!newEdu.school) return; const ed = [...(d.education || []), newEdu]; setDraft(x => ({ ...x, education: ed })); save({ education: ed }); setNewEdu({ school: '', degree: '', major: '', startYear: '', endYear: '' }); setAddingEdu(false); }} style={{ padding: '7px 14px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Add</button>
                    <button onClick={() => setAddingEdu(false)} style={{ padding: '7px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-mid)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                  </div>
                </div>
              )}
            </Card>
            ...
            <Card>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                ...
                <button onClick={() => setAddingProject(true)} style={{ padding: '6px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-dim)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>+ Add</button>
              </div>
              {(d.projects || []).map((p, i) => (
                <div key={i} style={{ padding: '12px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                    {(Array.isArray(p.techStack) ? p.techStack : []).map((t, j) => (
                      <span key={j} style={{ fontSize: 10, padding: '3px 8px', borderRadius: 0, border: '1px solid var(--border)', color: 'var(--text-mid)' }}>{t}</span>
                    ))}
                  </div>
                </div>
              ))}
              {addingProject && (
                <div style={{ border: '1px solid var(--border)', borderRadius: 0, padding: 16, background: 'var(--bg-subtle)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => { if (!newProject.title) return; const pr = [...(d.projects || []), { ...newProject, techStack: newProject.techStack.split(',').map(t => t.trim()), id: Date.now() }]; setDraft(x => ({ ...x, projects: pr })); save({ projects: pr }); setNewProject({ title: '', techStack: '', description: '', projectUrl: '', githubUrl: '' }); setAddingProject(false); }} style={{ padding: '7px 14px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Add Project</button>
                    <button onClick={() => setAddingProject(false)} style={{ padding: '7px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-mid)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                  </div>
                </div>
              )}
            </Card>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, marginBottom: 12 }}>
                {(d.certifications || []).map((c, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', fontSize: 12 }}>
                    🏅 {c}
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => { if (!newCert.trim()) return; const cer = [...(d.certifications || []), newCert.trim()]; setDraft(x => ({ ...x, certifications: cer })); save({ certifications: cer }); setNewCert(''); }}
                  style={{ padding: '0 16px', minWidth: 50, borderRadius: 0, border: 'none', background: newCert.trim() ? 'var(--green)' : 'var(--border-mid)', color: '#fff', fontSize: newCert.trim() ? 12 : 14, fontWeight: 600, cursor: newCert.trim() ? 'pointer' : 'default', fontFamily: 'inherit', transition: 'all 0.15s' }}>
                  {newCert.trim() ? 'Add' : '+'}
                </button>
              </div>
            </Card>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 12 }}>
                {(d.awards || []).map((a, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', fontSize: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => { if (!newAward.trim()) return; const aw = [...(d.awards || []), newAward.trim()]; setDraft(x => ({ ...x, awards: aw })); save({ awards: aw }); setNewAward(''); }}
                  style={{ padding: '0 16px', minWidth: 50, borderRadius: 0, border: 'none', background: newAward.trim() ? 'var(--green)' : 'var(--border-mid)', color: '#fff', fontSize: newAward.trim() ? 12 : 14, fontWeight: 600, cursor: newAward.trim() ? 'pointer' : 'default', fontFamily: 'inherit', transition: 'all 0.15s' }}>
                  {newAward.trim() ? 'Add' : '+'}
                </button>
              </div>
            </Card>
              ...
              {({ editing }) => (
                <>
                  {Object.entries(PLATFORM_LOGOS).map(([key, { label, color, svg }]) => (
                    <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 0, background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{svg}</div>
            ...
            <Card>
              ...
              {otherLinks.map((link, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-subtle)', marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 0, background: 'var(--bg-card)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 14 }}>🔗</div>
              ...
              <button onClick={() => { if (!newOtherLink.url.trim()) return; const nl = [...otherLinks, { label: newOtherLink.label || newOtherLink.url, url: newOtherLink.url }]; setOtherLinks(nl); save({ other_links: nl }); setNewOtherLink({ label: '', url: '' }); }}
                style={{ padding: '7px 16px', borderRadius: 0, border: 'none', background: newOtherLink.url.trim() ? 'var(--green)' : 'var(--border-mid)', color: '#fff', fontSize: newOtherLink.url.trim() ? 12 : 14, fontWeight: 600, cursor: newOtherLink.url.trim() ? 'pointer' : 'default', fontFamily: 'inherit', transition: 'all 0.15s' }}>
            ...
            <div style={{ maxWidth: 760 }}>
              ...
              {pendingSkill && (
                <>
                  ...
                  <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 401, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '28px 28px 24px', width: 360, boxShadow: '0 20px 60px rgba(0,0,0,0.18)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {LEVELS.map(lv => (
                        <button key={lv.v} onClick={() => confirmAdd(lv.v)} style={{
                          ...
                          padding: '10px 14px', borderRadius: 0, border: '1px solid var(--border)',
                    ...
                    <button onClick={() => setPendingSkill(null)} style={{ marginTop: 14, width: '100%', padding: '8px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-dim)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                  </div>
                </>
              )}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                ...
                <button onClick={() => save({ skills: d.skills || [] })} disabled={saving}
                  style={{ padding: '7px 16px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.6 : 1, fontFamily: 'inherit' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: skillSearch ? '1fr' : '180px 1fr', gap: 16, alignItems: 'start' }}>
                    ...
                    {SKILL_CATS_P.map(cat => {
                      ...
                      return (
                        <button key={cat} onClick={() => setSkillCat(cat)} className="hn-item" style={{
                          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                          padding: '9px 12px', borderRadius: 0, border: 'none', cursor: 'pointer', fontFamily: 'inherit',
                          ...
                          <span>{cat}</span>
                          {cnt > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', background: 'var(--green-tint)', borderRadius: 0, padding: '1px 7px' }}>{cnt}</span>}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                  {/* Picker */}
                  <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 0, padding: '16px 18px', marginBottom: 20 }}>
                        ...
                        return (
                          <button key={skill.name} onClick={() => { if (!added) setPendingSkill(skill.name); }}
                            style={{
                              ...
                              padding: '6px 12px', borderRadius: 0, fontSize: 12, fontFamily: 'inherit',
                  ...
                  {addedSkills.length > 0 ? (
                            ...
                            {skills.map((skill) => {
                              ...
                              return (
                                <div key={skill.id || skill.name}
                                  style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 10px 6px 8px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-card)', fontSize: 12, color: 'var(--text)', position: 'relative', cursor: 'default' }}
                                  ...
                                  <div style={{ display: 'flex', gap: 2, marginLeft: 2 }}>
                                    {[1, 2, 3, 4, 5].map(lv => (
                                      <div key={lv} style={{ width: 4, height: 4, background: skill.level >= lv ? levelColors[skill.level] : 'var(--border-mid)' }} />
                                    ))}
                                  </div>
                    ...
                    <div style={{ padding: '32px', textAlign: 'center', border: '2px dashed var(--border)', borderRadius: 0 }}>
                      ...
                      {editing ? (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, marginBottom: 18 }}>
                          {ROLE_OPTIONS_P.map(role => {
                            ...
                            return <button key={role} onClick={() => setDraft(x => ({ ...x, preferred_roles: sel ? (x.preferred_roles || []).filter(r => r !== role) : [...(x.preferred_roles || []), role] }))} style={{ padding: '9px 12px', borderRadius: 0, fontSize: 12, fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left', border: `1px solid ${sel ? 'var(--green)' : 'var(--border)'}`, background: sel ? 'var(--green-tint)' : 'transparent', color: sel ? 'var(--green-text)' : 'var(--text-mid)', fontWeight: sel ? 500 : 400 }}>{role}</button>;
                          })}
                        </div>
                      ) : (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 18 }}>
                          {(d.preferred_roles || []).length > 0 ? (d.preferred_roles || []).map(r => (
                            <span key={r} style={{ padding: '5px 12px', borderRadius: 0, fontSize: 12, background: 'var(--green-tint)', color: 'var(--green-text)', border: '1px solid var(--green)' }}>{r}</span>
          ...
          <div style={{ maxWidth: 720 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {(d.journey || []).map((j, i) => {
                ...
                return (
                  <div key={i} style={{ position: 'relative', paddingLeft: 40, marginBottom: 20 }}>
                    ...
                    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 0, padding: '16px 18px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                        <div style={{ flex: 1 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: tc.bg, padding: '2px 8px', borderRadius: 0, background: tc.tint, display: 'inline-block', marginBottom: 6 }}>{tc.label}</span>
              ...
              {(d.journey || []).length === 0 && !addingJourney && (
                <div style={{ padding: '48px 24px', textAlign: 'center', border: '2px dashed var(--border)', borderRadius: 0, marginBottom: 16 }}>
            ...
            {/* Add form */}
            {addingJourney ? (
              <div style={{ border: '1px solid var(--border)', borderRadius: 0, padding: 20, background: 'var(--bg-card)' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={() => setAddingJourney(false)} style={{ padding: '7px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-mid)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setAddingJourney(true)} style={{ padding: '10px 20px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>+ Add entry</button>
            )}
          </div>
        )}
        ...
        {activeSection === 'myhunt' && (
          <div style={{ maxWidth: 600 }}>
            ...
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 14 }}>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  {d.my_hunt && <button onClick={() => save({ my_hunt: d.my_hunt })} style={{ padding: '5px 12px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>}
                </div>
              </div>
            ...
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 14 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', letterSpacing: '0.1em', fontFamily: "'Instrument Serif', Georgia, serif" }}>02</span>
                <div>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  {d.philosophy && <button onClick={() => save({ philosophy: d.philosophy })} style={{ padding: '5px 12px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>}
                </div>
              </div>
            ...
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 14 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', letterSpacing: '0.1em', fontFamily: "'Instrument Serif', Georgia, serif" }}>03</span>
                <div>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  {d.inspirations && <button onClick={() => save({ inspirations: d.inspirations })} style={{ padding: '5px 12px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>}
                </div>
              </div>
            ...
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 14 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', letterSpacing: '0.1em', fontFamily: "'Instrument Serif', Georgia, serif" }}>04</span>
                <div>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  {d.life_outside && <button onClick={() => save({ life_outside: d.life_outside })} style={{ padding: '5px 12px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>}
                </div>
              </div>
            ...
            <div style={{ marginBottom: 48 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, marginBottom: 20 }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--green)', letterSpacing: '0.1em', fontFamily: "'Instrument Serif', Georgia, serif" }}>"</span>
                <div>
                ...
                <div style={{ marginLeft: 'auto' }}>
                  {d.quote && <button onClick={() => save({ quote: d.quote, quote_author: d.quote_author })} style={{ padding: '5px 12px', borderRadius: 0, border: 'none', background: 'var(--green)', color: '#fff', fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>}
                </div>
              </div>
              {d.quote ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <input style={{ ...inp_p, fontSize: 14, fontFamily: "'Instrument Serif', Georgia, serif", fontStyle: 'italic' }} value={d.quote || ''} placeholder="The people who are crazy enough to think they can change the world…" onChange={e => setDraft(x => ({ ...x, quote: e.target.value }))} />
        ...
        {activeSection === 'reviews' && (
          <div style={{ maxWidth: 640 }}>
            ...
            <div style={{ padding: '64px 32px', textAlign: 'center', border: '2px dashed var(--border)', borderRadius: 0, background: 'var(--bg-card)' }}>
        ...
        {activeSection === 'account' && (
          <div style={{ maxWidth: 560 }}>
            ...
            <Card>
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', borderRadius: 0, background: 'var(--bg-subtle)', border: '1px solid var(--border)', marginBottom: 20 }}>
                {d.avatar_url ? (
                ...
                <label style={{ fontSize: 12, padding: '6px 14px', borderRadius: 0, border: '1px solid var(--border)', cursor: 'pointer', color: 'var(--text-mid)', background: 'var(--bg-card)', fontFamily: 'inherit' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={handleChangeEmail} disabled={changingEmail} style={{ padding: '0 16px', borderRadius: 0, border: 'none', background: 'var(--text)', color: 'var(--bg)', fontSize: 12, cursor: changingEmail ? 'default' : 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', opacity: changingEmail ? 0.6 : 1 }}>
            ...
            <Card>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {['light', 'dark'].map(t => (
                  <button key={t} onClick={() => setTheme(t)} style={{ flex: 1, padding: '10px', borderRadius: 0, border: `1.5px solid ${theme === t ? 'var(--text)' : 'var(--border)'}`, background: theme === t ? 'var(--bg-subtle)' : 'transparent', cursor: 'pointer', fontSize: 13, fontWeight: theme === t ? 600 : 400, color: 'var(--text)', fontFamily: 'inherit' }}>
            ...
            <Card>
              ...
              <button onClick={handleSignOut}
                style={{ width: '100%', padding: '10px 14px', borderRadius: 0, border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" /><polyline points="16 17 21 12 16 7" /><line x1="21" y1="12" x2="9" y2="12" /></svg>
                Sign out
              </button>
              <button onClick={handlePauseAccount}
                style={{ width: '100%', padding: '10px 14px', borderRadius: 0, border: '1px solid var(--border)', background: d.account_paused ? 'var(--green-tint)' : 'var(--bg-card)', color: d.account_paused ? 'var(--green-text)' : 'var(--text)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ...
            <div style={{ padding: '16px 18px', borderRadius: 0, border: '1px solid var(--red)', background: 'var(--red-tint)' }}>
              ...
              <button onClick={() => { if (window.confirm('Delete your account? This cannot be undone.')) showToast('Delete coming soon', 'err'); }} style={{ padding: '7px 16px', borderRadius: 0, border: '1px solid var(--red)', background: 'transparent', color: 'var(--red)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Delete account</button>
            </div>
          </div>
        )}
