...
export function ExploreTab({
  ...
  // Decide what's causing an empty state — search vs filters
  ...
  return (
    <div style={{ flex: 1, display: 'flex', overflow: 'hidden', animation: 'hunt-fade-in 0.3s ease' }}>
      ...
      {/* Left: list/grid panel — hidden entirely when detail panel is maximized */}
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 24px 24px' }}>
            {allJobs.length === 0 ? (
              ...
              // Empty state — search/filter narrowed everything out
              <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-dim)', fontSize: '13px' }}>
                {hasSearch && !hasFilters ? (
                  <>
                    No results match your searches.{' '}
                    <button onClick={() => setSearchQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--green)', fontSize: '13px', textDecoration: 'underline' }}>Clear search</button>
                  </>
                ) : hasSearch && hasFilters ? (
                  <>
                    No results match your searches or filters.{' '}
                    <button onClick={() => { setSearchQuery(''); handleFilterChange('reset'); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--green)', fontSize: '13px', textDecoration: 'underline' }}>Clear all</button>
                  </>
                ) : (
                  <>
                    No results match your filters.{' '}
                    <button onClick={() => handleFilterChange('reset')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--green)', fontSize: '13px', textDecoration: 'underline' }}>Clear filters</button>
                  </>
                )}
              </div>
            ) : (
