...
//
// SETUP:
// 1. Drop into src/components/AdminJobCreator.jsx
// 2. App.jsx import:
//      import AdminJobCreator from './components/AdminJobCreator';
// 3. App.jsx route — PUBLIC (no auth guard, protected by password instead):
//      <Route path="/admin" element={<AdminJobCreator />} />
// 4. Replace YOUR_RECRUITER_ID_HERE with the UUID from:
//      SELECT id FROM recruiters WHERE email = 'admin@hunt.so';
// 5. Change ADMIN_PASSWORD to something only you know

import React, { useState, useEffect } from 'react';
import { Plus, X, Copy, Check, ExternalLink, Eye, EyeOff, Trash2, Link2 } from 'lucide-react';
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FAFAF8', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <div style={{ background: '#fff', border: '1px solid #EBEBEA', borderRadius: 12, padding: '36px 40px', width: '100%', maxWidth: 360, textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
        ...
        <button onClick={check} style={{ width: '100%', padding: '11px', borderRadius: 8, border: 'none', background: '#D85A30', color: '#fff', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>
          Enter
        </button>
      </div>
    </div>
...
// ─── Shareable link card ──────────────────────────────────────────────────────
function LinkCard({ job, onToggle, onDelete }) {
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #EBEBEA', borderRadius: 12, padding: '16px', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {/* WhatsApp share */}
        <button onClick={whatsapp} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px', borderRadius: 8, border: 'none', background: '#25D366', color: '#fff', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>
        ...
        {/* Open link */}
        <a href={url} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 10px', borderRadius: 8, border: '1px solid #EBEBEA', background: 'transparent', color: '#5A5A56', fontSize: 11, cursor: 'pointer', textDecoration: 'none' }} title="Open apply page">
...
function PostForm({ onSuccess, onCancel }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const normSkills = form.required_skills.map(sk => ({
        ...sk, weight: parseFloat((sk.weight / totalW).toFixed(3))
      }));
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {LOGO_EMOJIS.map(e => (
              <button key={e} onClick={() => set('logo')(e)} style={{ width: 34, height: 34, borderRadius: 6, fontSize: 17, border: `1.5px solid ${form.logo === e ? '#D85A30' : '#EBEBEA'}`, background: form.logo === e ? 'rgba(216,90,48,0.08)' : '#F5F5F2', cursor: 'pointer' }}>{e}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            ...
            <input style={inp} value={form.company} onChange={e => set('company')(e.target.value)} placeholder="TechFlow AI"
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          </div>
          <div>
            <Label required>Role title</Label>
            <input style={inp} value={form.role} onChange={e => set('role')(e.target.value)} placeholder="Backend Engineering Intern"
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          <div>
            ...
            <input style={inp} value={form.stipend} onChange={e => set('stipend')(e.target.value)} placeholder="₹25,000/month"
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          </div>
          <div>
            <Label required>Duration</Label>
            <input style={inp} value={form.duration} onChange={e => set('duration')(e.target.value)} placeholder="3 months"
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          </div>
          <div>
            <Label required>Location</Label>
            <input style={inp} value={form.location} onChange={e => set('location')(e.target.value)} placeholder="Remote"
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea style={{ ...inp, resize: 'none' }} rows={3} value={form.description} onChange={e => set('description')(e.target.value)} placeholder="What will the intern work on?"
            onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <input style={{ ...inp, flex: 1 }} value={skillInput} onChange={e => setSkillInput(e.target.value)} placeholder="Type skill + Enter" list="admin-skills" onKeyDown={e => e.key === 'Enter' && addSkill()}
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
              ...
              {form.required_skills.map(skill => (
                <div key={skill.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 7, border: '1px solid #EBEBEA', background: '#F5F5F2' }}>
                  ...
                  <div style={{ display: 'flex', gap: 3 }}>
                    {[1,2,3,4,5].map(lv => (
                      <button key={lv} onClick={() => setForm(f => ({ ...f, required_skills: f.required_skills.map(s => s.name === skill.name ? { ...s, level: lv } : s) }))} style={{ width: 22, height: 22, borderRadius: 4, fontSize: 10, fontWeight: 500, cursor: 'pointer', border: 'none', background: skill.level >= lv ? '#D85A30' : '#fff', color: skill.level >= lv ? '#fff' : '#9B9B97', outline: skill.level >= lv ? 'none' : '1px solid #EBEBEA' }}>{lv}</button>
                    ))}
                  </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
            <input style={{ ...inp, flex: 1 }} value={niceInput} onChange={e => setNiceInput(e.target.value)} placeholder="e.g. Docker, AWS" list="admin-skills" onKeyDown={e => e.key === 'Enter' && addNice()}
              onFocus={e => e.target.style.borderColor = '#D85A30'} onBlur={e => e.target.style.borderColor = '#EBEBEA'} />
          ...
          {form.nice_to_have.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
              {form.nice_to_have.map(s => (
                <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, padding: '3px 8px', borderRadius: 20, border: '1px solid #EBEBEA', color: '#5A5A56' }}>
                  {s}
                  <button onClick={() => setForm(f => ({ ...f, nice_to_have: f.nice_to_have.filter(x => x !== s) }))} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#9B9B97', display: 'flex', padding: 0 }}><X size={10} /></button>
                </span>
              ))}
            </div>
          )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 10, color: '#9B9B97', marginTop: 4 }}>
              {form.visibility === 'public' ? 'Shows in swipe feed.' : 'Only via share link.'}
            </p>
          </div>
          <div>
        ...
        {error && (
          <div style={{ padding: '9px 12px', borderRadius: 8, background: '#FDECEA', border: '1px solid #C0392B' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={handleSubmit} disabled={saving} style={{ flex: 2, padding: '11px', borderRadius: 8, border: 'none', background: saving ? '#9B9B97' : '#D85A30', color: '#fff', fontSize: 13, fontWeight: 500, cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit' }}>
            {saving ? 'Posting…' : 'Post & get link →'}
          </button>
        </div>
      </div>
    </div>
...
export default function AdminJobCreator() {
  ...
  const handleUnlock = () => {
    sessionStorage.setItem('hunt_admin', 'yes');
    setUnlocked(true);
  };
  ...
  const loadJobs = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('jobs').select('*')
        .eq('recruiter_id', ADMIN_RECRUITER_ID)
  ...
  const handlePostSuccess = (newJob) => {
    setShowForm(false);
    setJobs(j => [newJob, ...j]);
    showToast('Internship posted! Share link is ready.');
  };

  const handleToggle = async (job) => {
    try {
      const { data, error } = await supabase
        .from('jobs').update({ is_active: !job.is_active })
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF8', fontFamily: "'DM Sans', system-ui, sans-serif", WebkitFontSmoothing: 'antialiased' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 32px', borderBottom: '1px solid #EBEBEA', background: '#fff', position: 'sticky', top: 0, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#D85A30' }}>Admin · Job creator</span>
        </div>
      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '28px 24px 80px' }}>

        {/* Header */}
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 28, fontWeight: 400, color: '#0A0A0A', marginBottom: 6, letterSpacing: '-0.01em' }}>
            Your internships
          </h1>
          <p style={{ fontSize: 13, color: '#9B9B97' }}>
            Post roles, copy links, share on WhatsApp. Private roles are only accessible via link.
          </p>
        </div>
