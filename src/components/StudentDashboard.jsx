      ...
      <main style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {activeTab === 'network' && (
          <NetworkTab studentProfile={studentProfile} />
              ...
              <p style={{ fontSize: '13px', color: 'var(--text-dim)', marginBottom: '32px', lineHeight: 1.5 }}>
                Import your LinkedIn network or invite friends to join HUNT — referred students get priority matching.
              </p>

              {/* ── Import LinkedIn connections ── */}
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px 28px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                      <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>Import LinkedIn connections</p>
                      <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '4px', background: 'var(--green-tint)', color: 'var(--green)', fontWeight: 600 }}>Recommended</span>
                    </div>
                    <p style={{ fontSize: '12px', color: 'var(--text-dim)', lineHeight: 1.5, marginBottom: '16px' }}>
                      Export your connections from LinkedIn and upload the CSV. We'll show you which of your connections are already on HUNT or working at companies that are hiring.
                    </p>
                    <div style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '16px', padding: '10px 12px', borderRadius: '8px', background: 'var(--bg-subtle)', border: '1px solid var(--border)' }}>
                      <strong style={{ color: 'var(--text-mid)' }}>How to export:</strong> LinkedIn → Me → Settings → Data Privacy → Get a copy of your data → Select "Connections" → Request archive → Upload the CSV here
                    </div>
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <button style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: '#0A66C2', color: '#fff', fontSize: '12px', fontWeight: 600 }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="white"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                        Get my connections
                      </button>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '8px', border: '1px solid var(--border)', cursor: 'pointer', background: 'transparent', color: 'var(--text-mid)', fontSize: '12px', fontWeight: 500 }}>
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px 28px', marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)', marginBottom: '4px' }}>Invite friends via WhatsApp</p>
                    <p style={{ fontSize: '12px', color: 'var(--text-dim)', lineHeight: 1.5, marginBottom: '16px' }}>
                      Send your personal invite link to friends. When they sign up and get matched, you both move up the priority queue.
                    </p>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '12px' }}>
                      <div style={{ flex: 1, padding: '9px 12px', borderRadius: '8px', border: '1px solid var(--border)', background: 'var(--bg-subtle)', fontSize: '12px', color: 'var(--text-dim)', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        hunt.so/invite/{studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}
                      </div>
                      <button onClick={() => { navigator.clipboard?.writeText(`https://hunt.so/invite/${studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}`); }} style={{ padding: '9px 14px', borderRadius: '8px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-mid)', fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                        Copy link
                      </button>
                    </div>
                    <button onClick={() => { const msg = encodeURIComponent(`Hey! I'm using HUNT to find internships — it matches on skills, not college name. Join with my link: https://hunt.so/invite/${studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}`); window.open(`https://wa.me/?text=${msg}`, '_blank'); }}
                      style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '8px', border: 'none', cursor: 'pointer', background: '#25D366', color: '#fff', fontSize: '12px', fontWeight: 600 }}>
              ...
              {/* ── Invite via other channels ── */}
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px 28px', marginBottom: '16px' }}>
                <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)', marginBottom: '4px' }}>Share your invite link</p>
                <p style={{ fontSize: '12px', color: 'var(--text-dim)', lineHeight: 1.5, marginBottom: '16px' }}>Spread the word on other platforms</p>
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                  {/* Instagram */}
                  <button onClick={() => { navigator.clipboard?.writeText(`https://hunt.so/invite/${studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}`); }}
                    style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 16px', borderRadius: '8px', border: '1px solid var(--border)', cursor: 'pointer', background: 'transparent', color: 'var(--text-mid)', fontSize: '12px', fontWeight: 500 }}>
                  ...
                  {/* Twitter / X */}
                  <button onClick={() => { const msg = encodeURIComponent(`Found an internship platform that actually matches on skills 🎯\n\nNo more shotgun applications. Try HUNT → https://hunt.so/invite/${studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}`); window.open(`https://twitter.com/intent/tweet?text=${msg}`, '_blank'); }}
                    style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 16px', borderRadius: '8px', border: '1px solid var(--border)', cursor: 'pointer', background: 'transparent', color: 'var(--text-mid)', fontSize: '12px', fontWeight: 500 }}>
                  ...
                  {/* Email */}
                  <button onClick={() => { window.open(`mailto:?subject=Join me on HUNT&body=Hey! I'm using HUNT to find internships — it matches on your skills, not just your college name. Much better than Internshala. Check it out: https://hunt.so/invite/${studentProfile?.full_name?.split(' ')[0]?.toLowerCase() || 'you'}`, '_blank'); }}
                    style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 16px', borderRadius: '8px', border: '1px solid var(--border)', cursor: 'pointer', background: 'transparent', color: 'var(--text-mid)', fontSize: '12px', fontWeight: 500 }}>
              ...
              {/* ── Referral score ── */}
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '14px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'var(--green-tint)', border: '1px solid var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text)', marginBottom: '2px' }}>Your referral stats</p>
                  <p style={{ fontSize: '12px', color: 'var(--text-dim)' }}>0 friends joined · 0 priority boosts earned</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontFamily: "'Editorial New', Georgia, serif", fontSize: '24px', color: 'var(--green)' }}>0</p>
                  <p style={{ fontSize: '10px', color: 'var(--text-dim)' }}>referrals</p>
                </div>
              </div>
